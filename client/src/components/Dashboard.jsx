  ...
  return (
    <div className="min-h-screen flex">
      ...
      <nav className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <div className="p-4 border-t-4 border-primary space-y-2">
          <button
      ...
      <header className="topbar">
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="md:hidden p-2 -ml-2 hover:text-secondary transition-colors">
          <span className="material-symbols-outlined text-2xl">{sidebarOpen ? 'close' : 'menu'}</span>
        </button>
        <div className="font-headline-sm text-headline-sm font-bold text-primary uppercase tracking-tight">{topBarTitle}</div>
        <div className="flex items-center gap-4 text-primary">
          <button className="p-2 hover:bg-surface-variant rounded-full transition-colors">
            <span className="material-symbols-outlined hover:text-secondary">notifications</span>
          </button>
